...
- **New best BY selector route: compact denominator-pair history sink.** `denominator_pair_plus_50_sidecar_can_hold_raw_history_on_samples` / `denominator_pair_fixed_slack_schedule_50_sidecar_on_samples` show that the ordinary 256-bit BY denominator pair `(f,g)` can absorb raw branch history into its high-zero slack with a 50-bit sidecar over 8192 sampled secp256k1 denominators (adaptive max_converge=558/worst deficit=49; fixed per-step schedule worst=50). Including sign bits, selector/history is about `512+2+50=564` bits, inside the ~600 low-qubit allowance, and uses linear shift/add divsteps instead of full-ratio A-step inverses. This supersedes full-ratio as the most promising path. Next: prove/enforce the progress bound, design a reversible fixed-schedule history allocator for pair high-zero slack, and couple raw bits to scaled replay/reverse cleanup. Full-ratio remains a state-optimal but gate-blocked fallback (`h0` constmul okay at ~221k--443k; A-step dense; serial inverse proxy ~7.31M; W=32 still low valuation). See `src/point_add/by_sota_architecture.md`.
